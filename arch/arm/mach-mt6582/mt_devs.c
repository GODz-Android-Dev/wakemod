 ...
 */
u64 RESERVED_MEM_SIZE_FOR_PMEM = 0x1700000;
...
void mt_fixup(struct tag *tags, char **cmdline, struct meminfo *mi)
{
    ...
    if (mdesc)
    {
        ...
        printk(KERN_ALERT "mdesc->name=%s\n",mdesc->name);
    }
#endif
...
/* Charger IC driver                                                         */
/*=======================================================================*/
#if defined (CONFIG_MTK_SNB1058_SUPPORT) || defined (CONFIG_MTK_MAX8971_SUPPORT)
static struct platform_device charger_ic_dev = {
	.name = "charger_ic",
	.id   = -1,};
static struct platform_device charger_ic_i2c_dev = {
	.name = "charger_ic_i2c",
	.id   = -1,};	
#endif
...
"\n"
"  If you see this warning message, please update your uboot. [the situation is patched]\n"
"\n"
"******************************************************************************\n"
"   WARNING WARNING WARNING WARNING WARNING WARNING WARNING WARNING WARNING\n"
"******************************************************************************\n"
...
#if defined (CONFIG_MTK_SNB1058_SUPPORT) || defined (CONFIG_MTK_MAX8971_SUPPORT)
    retval = platform_device_register(&charger_ic_dev);
	if (retval != 0) {
		return retval;
	}
    retval = platform_device_register(&charger_ic_i2c_dev);
	if (retval != 0) {
		return retval;
	}
#endif
...
//    aee_dram_console_reserve_memory();
	...
	#if defined(CONFIG_MTK_EEMCS_DRIVER)
